...
-- Create table
create table SYS_EXTRAS_PAIRGROUP (
  ...
  CONSTRAINT SYS_EXTRAS_PAIRGROUP_UNIQUE UNIQUE(G_KEY),
  CONSTRAINT SYS_EXTRAS_PAIRGROUP_PK PRIMARY KEY(G_ID)
);     
-- Add comments to the table 
comment on table SYS_EXTRAS_PAIRGROUP  is '基础数据分组信息表';
-- Add comments to the columns 
comment on column SYS_EXTRAS_PAIRGROUP.G_ID  is '数据分组ID';
comment on column SYS_EXTRAS_PAIRGROUP.G_KEY  is '数据分组键';
comment on column SYS_EXTRAS_PAIRGROUP.G_TEXT  is '数据分组值';
comment on column SYS_EXTRAS_PAIRGROUP.G_STATUS  is '数据分组状态:（0:禁用|1：可用）';
comment on column SYS_EXTRAS_PAIRGROUP.G_ORDER  is '数据分组排序';
...
create table SYS_EXTRAS_PAIRVALUE (
  ...
  CONSTRAINT SYS_EXTRAS_PAIRVALUE_UNIQUE UNIQUE(D_GROUP, D_KEY),
  CONSTRAINT SYS_EXTRAS_PAIRVALUE_PK PRIMARY KEY(D_ID)
);
-- Add comments to the table 
...
create table SYS_EXTRAS_SETTINGS (
  ...
  D_STATUS  	NUMBER(1) default 1 not null,
  D_ORDER   	NUMBER(3) default 1 not null,
  CONSTRAINT SYS_EXTRAS_SETTINGS_UNIQUE UNIQUE(D_GROUP, D_KEY),
  CONSTRAINT SYS_EXTRAS_SETTINGS_PK PRIMARY KEY(D_ID)
);
-- Add comments to the table 
